...
- **Safety:** soft stall band (`aircraft_mvp.json` → `stall.speed_mps_below`, **55 m/s** at authoring) with heavy damping/sink bias; **hard fail** if **`stall_seconds`** in the snapshot reaches **`failure.stall_seconds`** (**4.0 s**).
